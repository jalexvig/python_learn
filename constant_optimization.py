...
# taken from http://code.activestate.com/recipes/277940-decorator-for-bindingconstants-at-compile-time/?in=lang-python


def _make_constants(f, builtin_only=False, stoplist=tuple(), verbose=False):
    try:
        co = f.__code__
    except AttributeError:
        return f        # Jython doesn't have a __code__ attribute.
    newcode = list(map(int, co.co_code))
...
def bind_all(mc, builtin_only=False, stoplist=tuple(),  verbose=False):
...
@_make_constants
def make_constants(builtin_only=False, stoplist=tuple(), verbose=False):
...
N = 10000
setup = 'from __main__ import sample, sample2; l = list(range(1000))'

t_opt = timeit.timeit('sample(l, 100)', setup=setup, number=N)
t_unopt = timeit.timeit('sample2(l, 100)', setup=setup, number=N)
